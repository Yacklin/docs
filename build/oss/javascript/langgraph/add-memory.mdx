...
That function sets up the schema necessary for the checkpoint to store data. It will also run any migrations on an existing database if you have upgraded your LangGraph version and the schema has changed.

We recommend calling this method in a way consistent with your organization's best practices for managing database schemas e.g. calling it when your application first starts
or including a call to it in any existing process you have that automatically runs database migrations.
